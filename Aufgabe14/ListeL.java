  ...
  public void sort2() {
    for (int i = 0; i < this.length() - 1; i++) {
      for (int j = 0; j < this.length() - i - 1; j++) {
        // Zwei Werte nebeneinander vergleichen
        ElementL<I> jElement = this.getElement(j);
        ElementL<I> nextElement = this.getElement(j + 1);
        if (jElement.inhalt.compareTo(nextElement.inhalt) > 0) {
          // Den Größeren Wert nach Rechts
          I help = nextElement.inhalt;
          nextElement.inhalt = jElement.inhalt;
          jElement.inhalt = help;
        }
      }
    }
